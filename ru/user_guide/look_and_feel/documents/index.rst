...
* **Переменные** содержат различную информацию из базы данных, например, имена покупателей или идентификационные номера заказов.
  ...
  Также, в шаблонах документов можно использовать :ref:`языковые переменные <language-variables>`. Например, если добавить в шаблон ``{{__("tracking_number")}}``, в готовый документ будет добавлен текст “Номер отслеживания”. Этот текст хранится в языковой переменной ``tracking_number``.
