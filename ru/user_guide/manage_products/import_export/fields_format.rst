...
Менять это вручную по всему файлу не нужно. Достаточно для поля, где есть HTML-элементы, задать модификатор ``concat('<![CDATA[', $value, ']]>')`` :doc:`на странице пресета импорта </user_guide/manage_products/import_export/advanced_product_import>`. Этот модификатор автоматически обернёт ``<![CDATA[...]]>`` вокруг всех значений в импортируемом поле.
