********************************************************************
Мобильное приложение для CS-Cart для маркетплейсов: Вопросы и ответы
********************************************************************
...
У CS-Cart для маркетплейсов адаптивный дизайн. Это значит, что сайт подстраивается под размеры экрана тех устройств, с которых его просматривают. Так покупатели могут оставлять заказы со смартфонов или планшетов.

Но есть и другой способ взаимодействия с маркетплейсом — через мобильное приложение. Это программа, которую покупатели устанавливают на свои мобильные устройства. Программа обменивается данными с маркетплейсом, так что покупатели могут увидеть, какие есть товары, заказать их, добавить в список желаемого, и т.д.

------------------------------------------------
Какие версии операционных систем поддерживаются? 
------------------------------------------------

Приложение поддерживает только следующие версии платформ:
...
* **Приложение — это один из видов рекламы.** Во-первых, логотип маркетплейса (или любое другое изображение на ваш выбор) появится в Apple App Store и в Google Play как иконка приложения. Во-вторых, если покупатель скачает приложение, то эту иконку он будет регулярно видеть на экране смартфона среди других приложений.
...
-----------------------------------------------------
Как приложение синхронизируется с моим маркетплейсом?
-----------------------------------------------------

В приложении есть все товары, которые есть у вас в маркетплейсе. Если вы добавите или удалите товары, категории, способы доставки или оплаты, то они также появятся или исчезнут из приложения.

Если у покупателя есть учётная запись в маркетплейсе, он сможет использовать её и в приложении. При регистрации в приложении покупатель также получает учётную запись в маркетплейсе.

Если покупатель добавляет товар в корзину или в список отложенных товаров в приложении, то эти действия переносятся в маркетплейс (и наоборот; синхронизация работает в обе стороны). Заказы из приложения ничем не отличаются от заказов с сайта: вы работаете с ними через панель администратора магазина, а покупатели всегда видят всю историю своих заказов (т.е. заказы и с сайта, и из мобильного приложения).
...
Способы оплаты в мобильном приложении расположены в том же порядке, что и в маркетплейсе. По умолчанию выбран первый способ из списка.
...
.. warning::   
    Мобильное приложение поддерживает модуль :doc:`/user_guide/addons/direct_customer_to_vendor_payments/index` из CS-Cart для маркетплейсов Плюс и Бизнес, начиная с версии CS-Cart для маркетплейсов 4.12.3.
...
Тексты мобильного приложения находятся в платформе в модуле **Мобильное приложение**. Когда появится поддержка других языков, то переводы на них будут выполняться сообществом и проверяться нашими партнёрами `в проекте на сервисе CrowdIn <https://crowdin.com/project/cs-cart-latest>`_.
...
#. Зарегистрируйтесь на `https://firebase.google.com <https://firebase.google.com>`_. Перейдите в консоль Firebase, кликнув в правом верхнем углу на кнопку **Go to console**. 

#. В консоли Firebase откройте **Settings** и создайте проект для Android и iOS.

#. Перейдите на вкладку **Service accounts** и нажмите кнопку **Generate new private key**. Вам будет предложено скачать JSON-файл с приватным ключом. Обязательно сохраните его. 

#. Скопируйте содержимое ключа и вставьте его в настройках модуля **Мобильное приложение**, в поле **Firebase Admin SDK Private key**. Модуль идёт в комплекте с Multi-Vendor, но не установлен по умолчанию.
...
* В Google Play: `Simbirsk Technologies Ltd <https://play.google.com/store/apps/collection/cluster?gsr=SjpqGE55SGlBMHRCK0VWb1AzWU1NUHErV1E9PbICHQobChdjb20uc2ltdGVjaC5tdWx0aXZlbmRvchAH:S:ANO1ljIrGKY>`_
...
#. Чтобы создать приложение и опубликовать его в Apple App Store и на Google Play, нам понадобится от вас кое-какая информация.
   ...
   В CS-Cart для маркетплейсов встроен модуль **Мобильное приложение** (не установлен по умолчанию). Этот модуль позволяет:
...
----------------------------------------------------------------
С какими версиями CS-Cart для маркетплейсов работает приложение?
----------------------------------------------------------------

Лучше всего использовать самую новую версию. Там всегда самые последние изменения в модуле **Мобильное приложение** и в механизме взаимодействия приложения с маркетплейсом.

Если у вас не последняя версия, просто упомяните это при обращении к нам, и мы поможем начать. Мы не обещаем интегрировать мобильное приложение в любую старую версию, но если вы сообщите нам в `Help Desk <https://helpdesk.cs-cart.com>`_ номер вашей версии, то мы изучим такую возможность и сообщим вам о результатах.
...
#. **Изображения:**
   ...
   * *Картинка для описания* — изображение с размером 1024x500, которое появится на странице вашего приложения в Google Play. Подробнее читайте в `инструкциях Google Play <https://support.google.com/googleplay/android-developer/answer/1078870?hl=ru>`_ (см. *Картинка для описания*).

   * *Заставка (вертикальная)* — изображение размером 2496x2496. Оно отобразится при запуске приложения на вашем мобильном телефоне. Оно отобразится при запуске приложения на вашем мобильном телефоне. Советуем использовать логотип вашего маркетплейса. Он должен быть в центре изображения и не занимать всю его ширину. Фон рекомендуем однотонный или градиентный, без мелких деталей.
...
#. **Ваша контактная информация:**
   ...
   * *Ссылка на политику конфиденциальности* — ссылка на страницу вашего маркетплейса, где находится ваша политика конфиденциальности.
...
После того, как приложение выпущено, оно будет автоматически :ref:`обмениваться данными с магазином <mobile-app-faq-sync>`. Но вы также можете внести изменения во внешний вид маркетплейса без нашей помощи и без необходимости для ваших покупателей обновлять приложение. Вот что вы можете изменить:
...
#. **Ссылки в меню Страницы в разделе Профиль.** Большая часть меню в разделе профиль остается неизменной, а пункты меню Страницы можно настраивать: добавлять и удалять оттуда пункты.

   .. important::

Изменять домашнюю страницу и меню Страницы в настройках профилянужно в панели администратора маркетплейса. Откройте страницу **Веб-сайт → Темы →  Макеты** и переключитесь на макет **MobileAppLayout**. Он появится только при установленном модуле **Мобильное приложение**.
...
Тексты приложения являются частью модуля **Мобильное приложение**, и их можно редактировать в панели администратора маркетплейса. Редактирование текстов работает так же, как :doc:`перевод CS-Cart для маркетплейсов </user_guide/look_and_feel/languages/translate>`:

#. Откройте страницу **Настройки → Тексты и языки → Редактирование содержимого**. 

#. Переключитесь на вкладку *Переводы мобильного приложения* в боковой панели справа.

#. Отредактируйте переводы. После сохранения ваши изменения автоматически появятся в магазине.

   .. fancybox:: img/mobile_app_texts.png
       :alt: Переводы мобильного приложения в панели администратора Multi-Vendor.
...
Названия блоков на главной странице приложения могут появляться, а могут не появляться. Это зависит от оболочки, которую вы выберете для блока в панели администратора вашего маркетплейса.

Откройте страницу **Веб-сайт → Редактировать макеты** и выберите макет **MobileAppLayout**. Перейдите на вкладку **Homepage** и нажмите на иконку шестерёнки у нужного блока, чтобы открыть его настройки. Так вы сможете выбрать оболочку для блока:
...
#. Откройте страницу **Веб-сайт → Темы → Редактировать макеты**.
...
#. Откроются настройки блока. Перейдите на вкладку **Контент**. Здесь вы сможете выбрать страницы, которые должны появиться в разделе Профиль → Страницы мобильного приложения.
