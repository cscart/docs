...
Основные свойства, которые были добавлены в новый "Центр обновлений":

* Больше нет SQL файлов. Вся работа с БД ведется через миграции.

* Модулям теперь можно указывать свои сервера обновлений и загружать свои пакеты обновлений.
...
.. image:: img/uc_interface.png
    :align: center
    :alt: Центр обновлений в CS-Cart и Multi-Vendor.
...
* **Add-on** — пакеты скачиваются через коннекторы модулей и с серверов, указанных в этих коннекторах.

Пакет обновлений можно также загрузить вручную, используя форму загрузки на странице "Центра обновлений" (кнопка **+**).
...
* ``file`` — имя файла (например, *some_upgrade_1.2.3.tgz*).
* ``name`` — заголовок пакета обновления (например: *"Обновление модуля Marketplace"*).
* ``description`` — описание. Что делает пакет обновлений, какие ошибки были исправлены, какие улучшения добавлены.
* ``from_version`` — с какой версии обновляется ядро/модуль.
* ``to_version`` — до какой версии обновляется ядро/модуль.
...
Если схема обновления проходит проверку, то будет создана схема */путь/к/установке/var/upgrade/packages/NAME/schema.json*. Где **NAME** — имя модуля или *core* (для ядра).
...
Коннектор соединяется со своим сервером обновлений, передает необходимую информацию и получает содержимое пакета, который сохраняет по переданному пути. В случае успешного сохранения коннектор должен вернуть ``array(true, '');``. Если призошла ошибка (не удалось подключиться к серверу, не смог сохранить и т.д.), возвращается ошибка ``array(false, __('text_uc_cant_download_package'))``.

После получения файла Upgrade Center App распаковывает и проверяет:
...
* В случае с пакетом обновления для модуля, проверяется, что этот пакет не пытается перезаписать core-файлы. Доступные пути для файлов модуля:
...
.. important::
    ...
    При установке языков из пакета обновлений будут установлены только новые языковые переменные. Старые обновлены не будут. **Если нужно обновить существующую языковую переменную, используйте новое имя или создайте миграцию.**

Папка **migrations** содержит phinx-`миграции <https://github.com/cakephp/phinx/blob/v0.4.3/docs/migrations.rst>`_. Применяются в порядке *TIMESTAMP* имени файла. Теоретически, миграции должны работать только с БД. На практике они работают в окружении CS-Cart и могут делать что угодно. 

.. important::

    Все же для работы с файлами нужно использовать *PRE/POST* скрипты.
...
Файл **package.json** описывает всю структуру пакета. Какие файлы входят, хеши обновляемых файлов, миграции, языки и т.д.
...
Каждый шаг установки обновления логгируется в файл *var/upgrade/[NAME]_log.txt*, где **NAME** — *core* или название модуля. В случае непредвиденной остановки обновления всегда можно узнать причину.
