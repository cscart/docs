...
The **Customers** section of the administration panel allows you to manage the user accounts of your store. A *user account* identifies a registered user and associates the user with the related data: order statistics, personal info, subscriptions, etc. 
...
* **Customer**—can log in to the storefront and buy products in your store. The storefront is normally available to non-registered users as well, but registered users are supposed to enjoy greater privileges. Besides, even registered users with a customer account may have different opportunities depending on the user group that they belong to.
...
If you search for specific users often (for example, users from a certain country), we recommend saving the search, and it will appear

.. image:: img/users.png
    :align: center
    :alt: Use the Advanced search to make working with customers easier.
...
* **Delete**—permanently deletes the user account.
  ...
  .. warning::
      ...
      Account deletion can't be undone without restoring a database backup. We recommend that you disable accounts instead of deleting them.
