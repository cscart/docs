...
[*] Orders: Changed the behavior for returning to the checkout page without paying (for example, using the “Back” button in the browser). Previously, the order was "deleted", and only got created after a successful payment. Now the order will remain with the “Failed” status, and a new order will be placed alongside it after a successful payment.
