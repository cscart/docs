...
[+] Multi-Vendor: Add-ons: Stores and pickup points: Filеtering by vendors was added for the list of stores and pickup points on the storefront.
...
[!] Core: JS: Warnings occured on PHP 7.3 when minifying JavaScript sources. Fixed.
...
[!] Layouts: The blocks from the "Default" layout page could be modified on the "Layout" tabe of the editing page of an object (product, page, etc) in some cases. Fixed.
...
[!] Multi-Vendor: Add-ons: Vendor plans: A vendor plan that was not avaliable for the specific storefront could still appear during vendor registration. Fixed.
...
[!] Multi-Vendor Plus: Add-ons: Common products for vendors: Database error could occurr if some vendors tried to start to selling a common product from the same category at the same time. Fixed.
...
[!] Products: Forbidden combinaitons: It was impossible to add one more forbidden option combination to product if at least one was already in the list. Fixed.
